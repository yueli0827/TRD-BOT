...
# Author : Simo Ryu
# Adapted for SD15 by Nick Stracke
...
class Attention(nn.Module):
    def __init__(self, inner_dim, cross_attention_dim=None, num_heads=8, dropout=0.0):
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.attn2 = Attention(hidden_size, 768)
...
class Transformer2DModel(nn.Module):
    def __init__(self, in_channels, out_channels, n_layers):
        ...
        self.proj_in = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
        self.transformer_blocks = nn.ModuleList([BasicTransformerBlock(out_channels) for _ in range(n_layers)])
        self.proj_out = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0)
...
class SD15UNetModel(nn.Module):
    def __init__(self):
        super(SD15UNetModel, self).__init__()
