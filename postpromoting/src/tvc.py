...
# Fast implmentation of Cv @ T @ Ck.T for GW component of tvc with O(NK) complexity
...
# tvc objective function gradients for mirro descent solver
...
# Sinkhorn projections for balanced tvc (balanced assignment for frames AND actions)
...
# tvc objective function evaluation
...
def tvc_objective(T, cost_matrix, eps, alpha, radius, ub_frames, ub_actions,
...
def segment_tvc(cost_matrix, mask=None, eps=0.07, alpha=0.3, radius=0.04, ub_frames=False,
    ...
    while True:
        with torch.no_grad():
            obj = tvc_objective(T, cost_matrix, eps, alpha, radius, ub_frames, ub_actions, lambda_frames,
                                lambda_actions, mask=mask)
